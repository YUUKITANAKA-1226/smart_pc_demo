...
def apply_input_event(payload: dict) -> None:
    if not INPUT_AVAILABLE or not isinstance(payload, dict):
        return

    event_type = _safe_text(payload.get("type"))

    if event_type == "mouse_abs":
        x = _safe_int(payload.get("x", 0), 0)
        y = _safe_int(payload.get("y", 0), 0)
        mouse.position = (x, y)
        return

    if event_type == "mouse_click":
        mapping = {"left": Button.left, "right": Button.right, "middle": Button.middle}
        button = _safe_text(payload.get("button")) or "left"
        mouse.click(mapping.get(button, Button.left), 1)
        return
    ...
    if event_type == "key":
        key_name = _safe_text(payload.get("key")).lower()
        special = {
            "enter": Key.enter,
